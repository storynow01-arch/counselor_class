...
files.forEach(file => {
  ...
  // Blue updates
  content = content.replace(/blue-50/g, 'indigo-50/50');
  ...
  content = content.replace(/blue-600/g, '[#4F46E5]'); // Important step: converting directly to hex form
  ...
  // Red updates
  content = content.replace(/red-100/g, 'red-50');

  // Specific HEX overrides
  ...
  content = content.replace(/bg-\[\#4F46E5\] hover:bg-indigo-700 text-white rounded-\[16px\] shadow-sm(.*?)>確認預約/g, 'bg-[#FB923C] hover:bg-orange-500 text-white rounded-[16px] shadow-sm$1>確認預約');
